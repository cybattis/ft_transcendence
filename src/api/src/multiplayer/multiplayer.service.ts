...
import { BallUpdate, GameRoom, MovementUpdate, ScoreUpdate, ServeUpdate } from "./types/multiplayer.types";
...
export class MultiplayerService {
   ...
   */
  public processMovementUpdate(client: AuthedSocket, update: MovementUpdate): void {
  ...
  public async processGoal(client: AuthedSocket): Promise<void> {
    ...
    if ((game.player1Score >= 11 && game.player1Score - game.player2Score >= 2)
  ...
  public async disconnectPlayer(client: AuthedSocket): Promise<void> {
    ...
    if (game) {
      ...
      console.log("[MULTIPLAYER] Player " + client.userId.toString() + " disconnected from game " + game.id.toString());
